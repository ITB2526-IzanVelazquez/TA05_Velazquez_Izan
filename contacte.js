...
function playPlaneAnimation() {
  // reinicia animación si se vuelve a enviar rápido
...
form.addEventListener("submit", (e) => {
  ...
  if (!fields.name.value.trim()) {
    errors.name.textContent = "Nombre obligatorio";
    ok = false;
  }
  if (!fields.surname.value.trim()) {
    errors.surname.textContent = "Apellido obligatorio";
    ok = false;
  }
  if (!fields.email.value.trim() || !isGmail(fields.email.value)) {
    errors.email.textContent = "Email @gmail.com obligatorio";
    ok = false;
  }
  if (!fields.subject.value.trim()) {
    errors.subject.textContent = "Asunto obligatorio";
    ok = false;
  }
  if (!fields.message.value.trim()) {
    errors.message.textContent = "Mensaje obligatorio";
    ok = false;
  }
